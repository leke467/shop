...
function EmeraldCatalog({ shop, products = [], onQuickView }) {
  ...
  const customCatalogues = extra.custom_catalogues || extra[`${extra.template_id || 'emerald'}_custom_catalogues`] || shop?.theme?.extra_tokens?.custom_catalogues || {}
            ...
            return (
              <div key={p.id || idx} className={`grid ${isLarge ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 md:grid-cols-3'} gap-6 items-center ${idx % 2 === 1 ? 'md:direction-rtl' : ''}`}>
                ...
                <div className={`${isLarge ? '' : 'md:col-span-2'} space-y-3 p-4`}>
                  <span className="text-xs text-emerald-600 uppercase tracking-wider">{getCategoryDisplay(p)}</span>
