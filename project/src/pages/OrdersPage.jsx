...
export default function OrdersPage() {
  ...
  const fetchDeliveryCodes = async (orderId) => {
    ...
    try {
      ...
      // data is { "group_id": "code" }
      setDeliveryCodes(prev => ({ ...prev, [orderId]: data }))
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        {orders.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
            ...
            <Link to="/explore/products" className="inline-flex items-center justify-center px-6 py-3 rounded-xl bg-primary-600 text-white font-semibold hover:bg-primary-700 transition-colors">
              Start Shopping
            </Link>
          </div>
        ) : (
          <div className="space-y-6">
            {orders.map(order => (
              <div key={order.public_id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
                <div className="p-4 sm:p-6 border-b border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <span className="font-semibold text-gray-900">Order #{order.public_id.split('-')[0].toUpperCase()}</span>
                    ...
                    <p className="text-sm text-gray-500">{new Date(order.created_at).toLocaleDateString()} • {order.groups?.length || 0} Shop(s)</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-500 mb-1">Total Amount</p>
                    <p className="font-bold text-gray-900 text-lg">₦{Number(order.grand_total).toLocaleString()}</p>
                  </div>
                </div>
                      ...
                      <div key={group.id} className="border border-gray-100 rounded-xl p-4">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 pb-4 border-b border-gray-50">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <Link to={`/shop/${group.shop}`} className="font-semibold text-gray-900 hover:text-primary-600 transition-colors">
                                {group.shop_name}
                              </Link>
                              <div className="text-sm text-gray-500">Status: <span className="font-medium text-gray-700">{group.status}</span></div>
                            </div>
                          </div>
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {group.escrow_status === 'held' && (
                              <button 
                                ...
                                className="px-3 py-1.5 text-sm font-medium rounded-lg bg-error-50 text-error-600 hover:bg-error-100 transition-colors"
                              >
                                Raise Dispute
                              </button>
                            )}
                            {group.escrow_status === 'disputed' && (
                              <span className="px-3 py-1.5 text-sm font-medium rounded-lg bg-error-100 text-error-700">
                                Disputed
                              </span>
                            )}
                          </div>
                        ...
                        <div className="space-y-3">
                          {group.items?.map(item => (
                            <div key={item.id} className="flex justify-between items-start">
                              ...
                              <p className="font-medium text-gray-900">₦{Number(item.line_total).toLocaleString()}</p>
                            </div>
                          ))}
                        </div>
                        ...
                        {deliveryCodes[order.public_id]?.[group.id] && (
                          <div className="mt-4 p-4 rounded-xl bg-primary-50 border border-primary-100 flex items-center justify-between">
                            <div>
                              <p className="text-sm font-semibold text-primary-900 mb-1">Delivery Confirmation Code</p>
                              <p className="text-xs text-primary-700 max-w-sm">
                                Give this code to the seller ONLY after you have received and inspected your items.
                              </p>
                            </div>
                            <div className="text-2xl font-mono font-bold tracking-widest text-primary-700 bg-white px-4 py-2 rounded-lg shadow-sm">
                              {deliveryCodes[order.public_id][group.id]}
                            </div>
                          </div>
                        )}
        ...
        {showDisputeModal && disputeGroup && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden">
              <div className="p-6">
                <h3 className="text-xl font-bold text-gray-900 mb-2">Raise Dispute</h3>
                ...
                <form onSubmit={handleDispute}>
                  ...
                  <div className="flex justify-end gap-3">
                    <button type="button" onClick={() => setShowDisputeModal(false)} className="px-5 py-2.5 text-gray-600 font-medium hover:bg-gray-100 rounded-xl transition-colors">Cancel</button>
                    <button type="submit" disabled={disputing || !disputeReason.trim()} className="px-5 py-2.5 bg-error-600 hover:bg-error-700 text-white font-medium rounded-xl transition-colors disabled:opacity-50">
