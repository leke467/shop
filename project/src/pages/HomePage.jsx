    ...
    <Link to={`/product/${product.slug || product.public_id}`}>
      ...
      >
        ...
        <div className="p-3.5 sm:p-4">
          <h3 className="font-semibold text-gray-900 text-sm sm:text-base truncate group-hover:text-primary-600 transition-colors">{product.name}</h3>
          <p className="text-xs text-gray-500 truncate mt-0.5">{product.shop_name || product.shop?.name || ''}</p>
          
          <div className="flex items-center justify-between gap-1.5 mt-2.5">
            <div className="flex items-center gap-1">
              <svg className="w-3.5 h-3.5 text-warning-400 fill-current shrink-0" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
              <span className="text-xs font-semibold text-gray-700">{Number(product.rating_average || 0).toFixed(1)}</span>
              {product.purchase_count > 0 && (
                <span className="text-[11px] text-gray-400 hidden sm:inline">• {product.purchase_count} sold</span>
              )}
            </div>
            <span className={`text-[10px] sm:text-[11px] font-medium px-1.5 py-0.5 rounded shrink-0 whitespace-nowrap ${
              isOutOfStock 
