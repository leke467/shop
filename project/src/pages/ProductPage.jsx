          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-4">
              <div className="aspect-square bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-sm relative">
                ...
                >
                  <svg
                    className={`w-6 h-6 transition-colors ${isInWishlist(productSlug || product.id)
            ...
            <div className="lg:pt-4">
              ...
              <div className="mt-6 flex items-center justify-between gap-3">
                ...
                <span className={`text-xs font-semibold px-3 py-1.5 rounded-full ${(product.is_out_of_stock || (product.inventory_quantity ?? 100) <= 0)
                    ...
                    {(product.custom_measurement_type === 'fashion' || product.custom_measurement_type === 'dimensions') && (
                      <div className="flex items-center bg-white border border-amber-300 rounded-lg p-0.5 text-xs font-semibold">
                        <button
                          ...
                          className={`px-2 py-1 rounded-md transition-all ${measurementUnit === 'inches' ? 'bg-amber-500 text-white shadow-sm' : 'text-gray-600 hover:text-gray-900'
                            }`}
                        >
                          Inches
                        </button>
                        <button
                          ...
                          className={`px-2 py-1 rounded-md transition-all ${measurementUnit === 'cm' ? 'bg-amber-500 text-white shadow-sm' : 'text-gray-600 hover:text-gray-900'
                            }`}
                        >
                          cm
                        </button>
              ...
              <div className="mt-8 flex flex-wrap sm:flex-nowrap items-center gap-3">
                ...
                <motion.button
                  ...
                  className={`flex-1 py-3.5 px-6 rounded-xl text-white font-semibold shadow-lg disabled:opacity-60 transition-all duration-300 flex items-center justify-center gap-2 ${(product.is_out_of_stock || (product.inventory_quantity ?? 100) <= 0)
                ...
                <button
                  onClick={handleWishlistToggle}
                  className={`p-3.5 rounded-xl border-2 transition-all flex items-center justify-center ${isInWishlist(productSlug || product.id)
                ...
                <div className="flex gap-1 mb-6">
                  {['description', 'reviews'].map(t => (
                    <button
                      key={t}
                      onClick={() => setTab(t)}
                      className={`px-5 py-2.5 rounded-xl text-sm font-medium transition-all ${tab === t ? 'bg-gray-900 text-white' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'
                        }`}
                    >
