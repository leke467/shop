...
export default function GoogleLoginButton({ onSuccess, onError, text = "Continue with Google" }) {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      if (window.google?.accounts?.id) {
        ...
        // Trigger Google One-Tap Account Selector sheet on mobile / desktop
  ...
  return (
    <div className="w-full space-y-2">
      ...
      {/* Official Google Sign-In button container (displays Google's native account chooser) */}
      <div id="google-signin-btn-div" className="w-full flex justify-center min-h-[44px]"></div>

      {/* Fallback button if Google SDK fails to load or no client ID configured */}
      {!googleClientId && (
        <button
          type="button"
      ...
      {/* Demo Modal fallback only if Google API is unconfigured */}
      <AnimatePresence>
        {showModal && !googleClientId && (
