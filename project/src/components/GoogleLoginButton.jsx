...
export default function GoogleLoginButton({ onSuccess, onError, text = "Continue with Google" }) {
  ...
  // Fallback demo/prompt button if VITE_GOOGLE_CLIENT_ID is not configured yet
  const handleFallbackClick = () => {
    if (!googleClientId) {
      const mockEmail = prompt("Google Sign-In Setup Mode:\nEnter your Google Email to test instant social registration:", "user@gmail.com")
      if (!mockEmail) return
      setLoading(true)
      // Simulate ID token for dev/test setup
      googleLogin(`mock_google_id_token_${mockEmail}`)
        .then(() => {
          if (onSuccess) onSuccess()
          else navigate('/')
        })
